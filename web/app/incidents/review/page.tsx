        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="space-y-3">
            <div className="flex gap-2">
              {["pending-verification", "assigned", "resolved"].map(
                (status) => (
                  <button
                    key={status}
                ...
                filteredIncidents.map((inc) => (
                  ...
                  >
                    <p className="font-semibold text-sm">
                      INC-{inc.id.substring(0, 8).toUpperCase()}
                    </p>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-sm text-gray-400 mb-2">
                      INC-{current.id.substring(0, 8).toUpperCase()}
                    </p>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        type="button"
                        onClick={() =>
                          handleStatusUpdate(current.id, "assigned")
                        }
                        disabled={isUpdating}
                        className="py-3 bg-green-900/30 text-green-400 border border-green-700 rounded-lg hover:bg-green-900/50 transition font-semibold disabled:opacity-50"
                      >
                        Approve
                      </button>
                      <button
                        type="button"
                        onClick={() =>
                          handleStatusUpdate(current.id, "resolved")
                        }
                        disabled={isUpdating}
                        className="py-3 bg-red-900/30 text-red-400 border border-red-700 rounded-lg hover:bg-red-900/50 transition font-semibold disabled:opacity-50"
                      >
                        Reject
                      </button>
                    </div>
                    ...
                    {current.status === "assigned" && (
                      <div className="mt-4">
                        <button
                          ...
                          disabled={isUpdating}
                          className="w-full py-3 bg-green-900/30 text-green-400 border border-green-700 rounded-lg hover:bg-green-900/50 transition font-semibold disabled:opacity-50"
                        >
                          Mark Resolved
                        </button>
                      </div>
